...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def cut(text: str, width: int = 40) -> str:
        text = " ".join(str(text).split())   # gộp xuống dòng thành 1 dòng
        return text if len(text) <= width else text[: width - 3] + "..."

    headers = ["Prompt", "GPT-4o Response", "Mini Response",
               "GPT-4o Latency", "Mini Latency"]
    rows = [
        [cut(r["prompt"]), cut(r["gpt4o_response"]), cut(r["mini_response"]),
         f"{r['gpt4o_latency']:.2f}s", f"{r['mini_latency']:.2f}s"]
        for r in results
    ]
...
if __name__ == "__main__":
    ...
    stats = run_assistant(
        persona="Bạn là trợ giảng thân thiện của khóa AI Practical Competency. "
                "Luôn trả lời bằng tiếng Việt, ngắn gọn tối đa 3 câu. "
                "Nếu câu hỏi nằm ngoài chủ đề AI/lập trình, lịch sự từ chối "
                "và gợi ý quay lại bài học. "
                "Nếu không chắc chắn, hãy nói rõ là không chắc thay vì bịa.",
    )
    print("\n--- Thống kê phiên chat ---")
